...
The architecture shares a single BERT encoder(`transformers.BertModel` or `self.bert`) and branches into:
    * Intent head(self.intent_classifier): classifies the overall utterance (linear layer on top of the CLS embedding).
    * Slot head(self.slot_classifier)    : predicts a slot label for every token in the sequence (token-level linear layer).

Both heads can be trained simultaneously with a joint loss such as `loss.JointLoss`.
